...
GKC_legal %>% 
  group_by(string) %>%
  summarize(CPUEbar = mean(std_CPUE), Csd = sd(std_CPUE)) %>%
  mutate(Cse =  Csd/ sqrt(5), CV = (100*(Csd/CPUEbar)), crab_day = CPUEbar*24, base = CV-16) -> string_CPUE_legal
#calculate SD, SE, and CV
#CV is the % sd to the average.  
...
ggplot(string_CPUE_legal, aes(x = string, y = CPUEbar)) + geom_errorbar(aes(ymin=CPUEbar - Cse, ymax = CPUEbar+Cse), 
                                                                  width =0.1) + geom_point()
#plot crab per day
ggplot(string_CPUE_legal, aes(x = string, y = crab_day)) + geom_errorbar(aes(ymin=crab_day - (24*Cse), ymax = crab_day+ (24*Cse)), 
                                                                        width =0.1) + geom_point()
#CPUE by region
# this is the population mean 
string_CPUE_legal %>%
